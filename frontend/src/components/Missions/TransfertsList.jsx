// src/components/Missions/TransfertsList.jsx
    ...
    (async () => {
      try {
        ...
        const results = Array.isArray(data?.results)
          ...
          ? data
          : [];
        if (mounted) setMissions(results);
      } catch (e) {
  ...
  return (
    <div className="page-content p-4 bg-light min-vh-100">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          <h2 className="fw-bold text-dark mb-1">Mes Transferts</h2>
        ...
        <div className="d-flex gap-2">
          <button
            className="btn btn-outline-secondary d-flex align-items-center gap-2 shadow-sm px-3 py-2"
            onClick={() => nav("/missions/transferts/archive")}
      ...
      <div className="row g-4">
        <div className="col-lg-3">
          <div className="card border-0 shadow-sm rounded-4 p-3">
            ...
            <div className="small text-muted mt-3">
              Ici: missions <span className="fw-semibold">à venir / en cours</span>. <br />
              Une mission sort d’ici quand <span className="fw-semibold">fin + 120min</span> est dépassée.
            </div>
          </div>
        </div>
        ...
        <div className="col-lg-9">
            ...
            <div className="table-responsive">
              <table className="table table-hover align-middle mb-0">
                <thead className="bg-light">
                  <tr>
                    <th className="ps-4 border-0 text-muted small py-3" style={{ width: 230 }}>
                      ACTIONS
                    </th>
                  ...
                  {loading ? (
                    ...
                    filtered.map((m) => (
                      <tr key={m.id} className="border-bottom">
                        {/* ✅ ACTIONS: gauche = annuler&remplacer / droite = pdf */}
                        <td className="ps-4">
                          <div className="d-flex align-items-center justify-content-between gap-2">
                            <button
                              className="btn btn-sm btn-outline-danger d-flex align-items-center gap-2"
                              onClick={() => nav(`/missions/${m.id}/replace`)}
                              title="Annuler & remplacer les ressources"
                            >
                              <FaExchangeAlt /> Annuler & remplacer
                            </button>
          ...
          <div className="small text-muted mt-3">
            Astuce: les missions passées basculent automatiquement dans “Archives” après fin + 120min.
          </div>
        </div>
      </div>
    </div>
