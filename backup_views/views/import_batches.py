...
class ActiveBatchItemsAPIView(APIView):
    ...
    def get(self, request):
        ...
        batch_id = request.query_params.get(
            "batch"
        )  # ← optionnel : forcer un batch précis
        ...
        def _remaining_items_qs(batch):
            return (
                ...
                .filter(used=False)  # non encore utilisés dans une fiche
                .order_by("id")
            )
        ...
        # 1) Si batch explicite → prioritaire
        if batch_id:
            try:
            ...
            if agence_id and str(b.agence_id) != str(agence_id):
                # batch d'une autre agence → vide pour sécurité
                return Response([], status=200)
            ...
            if items_qs.exists():
                ...
                # rien dans ce batch → vide
                return Response([], status=200)

        # 2) Sinon, on remonte du plus récent au plus ancien pour l'agence donnée
        else:
            if not agence_id:
                return Response([], status=200)

            for b in ImportBatch.objects.filter(agence_id=agence_id).order_by(
                "-created_at", "-id"
            ):
                items_qs = _remaining_items_qs(b)
                if items_qs.exists():
