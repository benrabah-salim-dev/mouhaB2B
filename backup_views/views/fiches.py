...
class FicheMouvementListAPIView(APIView):
    permission_classes = [IsAuthenticated]

    def get(self, request):
        # Simple liste (facultatif)
        qs = FicheMouvement.objects.all().order_by("-created_at")
        return Response(FicheMouvementListSerializer(qs, many=True).data, status=200)


class FicheMouvementDetailAPIView(APIView):
    permission_classes = [IsAuthenticated]

    def get(self, request, fiche_id):
        fiche = get_object_or_404(FicheMouvement, id=fiche_id)
        return Response(FicheMouvementDetailSerializer(fiche).data, status=200)


class CreerFicheMouvementAPIView(APIView):
    """
    Crée UNE SEULE fiche à partir d'une sélection de dossiers.
    Marque ces dossiers comme `traite=True`.
    """
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request):
        ...
        type_code = (request.data.get("type") or "").strip().upper()  # "A" | "D"
        ...
        if not agence_id or not dossier_ids:
            return Response({"error": "Paramètres requis: agence, dossier_ids."}, status=400)

        agence = get_object_or_404(AgenceVoyage, id=agence_id)
        _ensure_same_agence_or_superadmin(request, agence.id)

        # Dossiers éligibles: même agence, pas déjà dans une fiche, et pas déjà traités
        dossiers = list(
            Dossier.objects.filter(id__in=dossier_ids, agence_id=agence.id, traite=False)
            .exclude(fiche_items__isnull=False)
        )
        if not dossiers:
            return Response({"error": "Aucun dossier éligible."}, status=400)
        ...
        Dossier.objects.filter(id__in=[d.id for d in dossiers]).update(traite=True)

        return Response({"ok": True, "fiche_id": fiche.id, "count": len(dossiers)}, status=201)
