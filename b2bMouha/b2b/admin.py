...
class ImportBatchAdmin(admin.ModelAdmin):
    list_display = ("id", "agence", "user", "created_at", "label")
    list_filter = ("agence", "created_at")
    search_fields = ("id", "label")


@admin.register(ImportBatchItem)
class ImportBatchItemAdmin(admin.ModelAdmin):
    list_display = ("batch", "dossier")
    list_filter = ("batch",)
    search_fields = ("batch__id", "dossier__id")
...
class VehicleOfferAdmin(admin.ModelAdmin):
    ...
    def trajet_display(self, obj):
        if obj.origin_zone or obj.destination_zone:
            ...
            b = (
                obj.destination_zone.nom
                if obj.destination_zone
                else (obj.destination or "-")
            )
        else:
            a = obj.origin or "-"
