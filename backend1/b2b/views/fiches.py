...
from b2b.serializers import (
    FicheMouvementSerializer, FicheMouvementItemSerializer, FicheMouvementDetailSerializer
)
from .helpers import (
...
class FichesMouvementListAPIView(APIView):
    ...
    Sortie par fiche :
      ...
      - hotels     = [{name, pax}] (liste complète)
      - pax        = somme pax (arrivée/départ)
      - clients    = concat courte (dedup)
      - observation= concat courte (dedup)
    ...
    def get(self, request):
        ...
        for fiche in qs.order_by("-date", "-id"):
            ...
            # Récupérations sûres (évite FK cassées)
            item_ids = list(
                FicheMouvementItem.objects
                .filter(fiche_id=fiche.id, dossier_id__isnull=False)
                .values_list("dossier_id", flat=True)
            )
            d_by_id = {
                d.id: d
                for d in Dossier.objects.filter(id__in=item_ids).select_related("hotel")
            }
            dossiers = [d_by_id[i] for i in item_ids if i in d_by_id]

            t = fiche.type
            apt = fiche.aeroport or ""

            # date_debut = min des heures connues
            heures = []
            for d in dossiers:
            ...
            rows.append({
                ...
                "date_fin": date_fin,
                "hotel": hotel_display,
                "hotels": hotels_list,   # ← liste complète d’hôtels demandée
...
def ordre_mission_pdf(request, ordre_id):
    ...
    elements.append(Paragraph(f"Départ : {fmt_dt(ordre.date_depart)}", style_normal))
    elements.append(Paragraph(f"Retour : {fmt_dt(ordre.date_retour)}", style_normal))
