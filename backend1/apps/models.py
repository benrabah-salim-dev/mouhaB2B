...
from apps.services.geocoding import lookup_hotel_address
...
class AgenceVoyage(models.Model):
    """
    Agence "finale" créée après validation d'une AgencyApplication.
    Champs alignés sur AgencyApplication.
    """
    ...
    # ===== Entreprise =====
    legal_name = models.CharField("Raison sociale", max_length=255)
    ...
    logo_file = models.ImageField(upload_to="agencies/logos/", blank=True, null=True)
    ...
    rep_date_naissance = models.DateField(null=True, blank=True)

    rep_photo_file = models.ImageField(upload_to="agencies/reps/", blank=True, null=True)
...
class AgencyApplication(models.Model):
    ...
    rne = models.CharField(max_length=100, blank=True, null=True)
    code_fiscal = models.CharField(max_length=100, blank=True, null=True)
    code_categorie = models.CharField(max_length=100, blank=True, null=True)
    etab_secondaire = models.CharField(max_length=100, blank=True, null=True)
    ...
    otp_delivery = models.CharField(max_length=10, choices=OTP_DELIVERY_CHOICES, default="email")
    ...
    def __str__(self):
        return f"Demande #{self.pk} - {self.legal_name} ({self.statut})"


    def approve(self, decided_by: User | None = None):
        if self.statut == "validee" and self.agence_id:
            return self.agence, self.agence.user, None
        ...
        with transaction.atomic():
            ...
            if not user:
                base_username = (email.split("@")[0] if email else f"agence{self.id}")
            ...
            # ---------- PROFILE ----------
            profile, _ = Profile.objects.get_or_create(user=user, defaults={"role": "adminagence"})
            ...
            agence, created = AgenceVoyage.objects.get_or_create(user=user, defaults=defaults_agence)

            # si déjà existante -> on complète / met à jour
            if not created:
                changed = False
                for field, value in defaults_agence.items():
                    # ici on choisit : soit on remplit si vide, soit on écrase
                    # version "safe": on ne remplace que si champ vide
                    if value is not None and getattr(agence, field) in (None, "", []):
...
class Profile(models.Model):
    ...
    agence = models.ForeignKey("apps.AgenceVoyage", on_delete=models.SET_NULL, null=True, blank=True)
...
class Succursale(models.Model):
    ...
    class Meta:
        ordering = ["agence__legal_name", "nom"]
...
class Hotel(models.Model):
    ...
    zone = models.ForeignKey("apps.Zone", on_delete=models.SET_NULL, null=True, blank=True, related_name="hotels")
    agence = models.ForeignKey("apps.AgenceVoyage", on_delete=models.SET_NULL, null=True, blank=True, related_name="hotels")
...
class Dossier(models.Model):
    agence = models.ForeignKey("apps.AgenceVoyage", on_delete=models.CASCADE)
    ...
    hotel_fk = models.ForeignKey("apps.Hotel", null=True, blank=True, on_delete=models.SET_NULL)
    zone_fk = models.ForeignKey("apps.Zone", null=True, blank=True, on_delete=models.SET_NULL)
...
class Vehicule(models.Model):
    ...
    class Meta:
        ordering = ["agence__legal_name", "immatriculation"]
...
class Chauffeur(models.Model):
    ...
    class Meta:
        ordering = ["agence__legal_name", "nom", "prenom"]
...
class HistoriqueTransformation(models.Model):
    ...
    fiche_de_mouvement = models.ForeignKey("apps.FicheMouvement", on_delete=models.SET_NULL, null=True, blank=True)
    mission = models.ForeignKey(
    "Mission",
    null=True,
    blank=True,
