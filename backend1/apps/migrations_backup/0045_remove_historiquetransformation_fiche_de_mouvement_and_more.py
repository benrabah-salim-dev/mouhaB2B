...
def _ensure_dossier_id_exists(schema_editor):
    ...
    # 2) ajouter une FK si elle n’existe pas (nom standard Django souvent: b2b_historiquetransformation_dossier_id_... mais on met un nom fixe)
    fk_name = "fk_historiquetransformation_dossier_id"
    if not _fk_exists(schema_editor, table, fk_name):
        # index d’abord (MySQL aime bien)
        with schema_editor.connection.cursor() as cursor:
            cursor.execute(
...
def forwards(apps, schema_editor):
    # Si MySQL dit “Can't DROP COLUMN … check it exists” => on drop seulement si présent
    _drop_column_if_exists(schema_editor, "b2b_historiquetransformation", "fiche_de_mouvement_id")

    # Ton erreur actuelle: dossier_id absent => on le crée avant l’AlterField logique
...
class Migration(migrations.Migration):
    ...
    dependencies = [
    ("b2b", "0044_alter_fichemouvement_options_and_more"),
]
    ...
    operations = [
        migrations.SeparateDatabaseAndState(
            ...
            state_operations=[
                # ce que Django “croit” niveau modèles (state)
